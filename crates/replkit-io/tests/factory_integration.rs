...
use replkit_io::{create_console_input, create_console_io, create_console_output, BackendType};
    ...
    match create_console_input() {
        Ok(input) => {
            ...
            #[cfg(windows)]
            {
                ...
                assert!(
                    caps.backend_type == BackendType::WindowsVt
...
fn test_create_console_io() {
    match create_console_io() {
        Ok((input, output)) => {
            ...
            println!(
                "Created console I/O pair: {} input, {} output",

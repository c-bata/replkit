...
This example demonstrates how to use the Python bindings for the prompt-core
...
def create_statistics_callback() -> Callable[[Any], None]:
    ...
    def stats_callback(event: Any):
        ...
        elif hasattr(prompt, 'Key') and event.key in [prompt.Key.Up, prompt.Key.Down, prompt.Key.Left, prompt.Key.Right,
                          prompt.Key.Home, prompt.Key.End, prompt.Key.PageUp, prompt.Key.PageDown]:
        ...
        elif hasattr(prompt, 'Key') and event.key in [prompt.Key.BracketedPaste, prompt.Key.Vt100MouseEvent, 
                          prompt.Key.CPRResponse]:
